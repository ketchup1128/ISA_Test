/*
 * TensorComp tc.st.mma.acc ISA Test - Test 2: Normal Mode (M2=2, N2=2)
 * =====================================================================
 * 
...
void test_2_normal_mode_2x2(void) {
    ...
    uint32_t rs2_config_0 = MAKE_RS2(0, 0, 0);
    uint32_t rs2_config_1 = MAKE_RS2(2, 2, 0);
    uint32_t rs2_config_2 = MAKE_RS2(4, 4, 0);
    uint32_t rs2_config_3 = MAKE_RS2(6, 6, 0);
    ...
    // Load trs3_data into tr2 (Left Data - 2x mode)
    uint32_t addr_trs3 = L1M_BASE_ADDR + 2048;
    PRINT("Loading trs3_data (0x%08X) into tr2\n", addr_trs3);
    T_LD(0, 0, addr_trs3);
    
    // Load trs4_data into tr3 (Top Data - 2x mode)
    ...
    TC_ST_MMA_ACC(acc_addr, 0, 1, rs2_config_0);
    PRINT("rs2_config: 0x%08X\n", rs2_config_1);
    TC_ST_MMA_ACC(acc_addr, 0, 1, rs2_config_1);
    PRINT("rs2_config: 0x%08X\n", rs2_config_2);
    TC_ST_MMA_ACC(acc_addr, 0, 1, rs2_config_2);
    PRINT("rs2_config: 0x%08X\n", rs2_config_3);
    TC_ST_MMA_ACC(acc_addr, 0, 1, rs2_config_3);
...
int main(void) {
    PRINT("======================================================================\n");
    PRINT("TensorComp tc.st.mma.acc ISA Test - Test 2: Normal Mode (2x2)\n");
    PRINT("======================================================================\n");
    PRINT("\nInstruction: tc.st.mma.acc - Matrix Multiply-Accumulate\n");
