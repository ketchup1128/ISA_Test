...
void test_3_top_only_4x1(void) {
    PRINT("\n=== Test 3: tc.st.add Top Data Only (4x1) ===\n");
    ...
    CSR_WRITE(CSR_TWIDTH, 128);
    CSR_WRITE(CSR_TLENGTH, MAKE_TLENGTH(0, 1, 0));  // left_len=0, top_len=1
    
    uint32_t acc_addr = ACC_BUFFER_ADDR;
    uint32_t rs2_config = MAKE_RS2(0, 0, 0);
    
    PRINT("tlength: 0x%08X (left_len=0, top_len=1)\n", MAKE_TLENGTH(0, 1, 0));
    PRINT("Computing: acc[m][n] = Top[n] (broadcast vertically)\n");
...
void test_4_top_only_2x2(void) {
    PRINT("\n=== Test 4: tc.st.add Top Data Only (2x2) ===\n");
...
void test_5_left_only_4x1(void) {
    PRINT("\n=== Test 5: tc.st.add Left Data Only (4x1) ===\n");
    ...
    CSR_WRITE(CSR_TWIDTH, 128);
    CSR_WRITE(CSR_TLENGTH, MAKE_TLENGTH(4, 0, 0));  // left_len=4, top_len=0
    
    uint32_t acc_addr = ACC_BUFFER_ADDR;
    uint32_t rs2_config = MAKE_RS2(0, 0, 0);
    
    PRINT("tlength: 0x%08X (left_len=4, top_len=0)\n", MAKE_TLENGTH(4, 0, 0));
    PRINT("Computing: acc[m][n] = Left[m] (broadcast horizontally)\n");
...
void test_6_left_only_2x2(void) {
    PRINT("\n=== Test 6: tc.st.add Left Data Only (2x2) ===\n");
...
int main(void) {
    ...
    test_3_top_only_4x1();
    test_4_top_only_2x2();
    test_5_left_only_4x1();
    test_6_left_only_2x2();
